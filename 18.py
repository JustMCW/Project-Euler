...
# for i,r in enumerate(trig_n):
#     r = r.split()
#     if i == len(trig_n)-1:
#         print(r)
#         break
#     mapped = list( map ( lambda i: int(i), r) )
#     mini = min(mapped)
#     mapped.remove(mini)
#     trig_n[i+1]=' '.join(list(map(lambda x,y:str(int(x)+int(y)), mapped,trig_n[i+1].split())))
#     print(trig_n[i+1])
array = []
def rec(x,y,v):
    v += int(trig_n[y].split()[x])
    if y == len(trig_n)-1:
